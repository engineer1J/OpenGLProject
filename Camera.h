...
public:
  ...
  void move_obj();
  void remove();
  void remove(float x_position, float z_position, float y_position);
  void Check_collision(float x_position, float z_position, float y_position);
...
private:
  ...
  glm::vec3	right_dir_;    // right direction of the camera    (it should be a unit vector whose length is 1)
	...
	float fovy_;
	float roll_;    //add var of roll
	float pitch_;   //add var of pitch
};
